...
public class TechNewsServiceImpl implements TechNewsService {
        ...
        if(hasKeyNewsMap) {
            if(0 == DateTimeUtils.checkTimeHourPeriod(calendarNow, hours)){
                ...
                if(1 == DateTimeUtils.checkTimeHourPeriod(calendarNow, hours)){
                    ...
                    String fileKey = latestNumber + ".png";
                    String outputPathAndFilename = QICHACHA_NEWS_PATH_PREFIX + fileKey;
                    ...
                    try {
                        if(PhantomjsUtils.screenCapture(QICHACHA_MORNING_NEWS_URL+latestNumber, outputPathAndFilename, 650, 6000, 1.3f, 5000)){
                            ...
                            if(AliOSSUtils.putFileObject(outputPathAndFilename, OSS_FOLDER_KEY, fileKey)){
                                log.info("[function] 企查查日报 {} 截图上传至OSS完成。", fileKey);
                            } else {
                                log.warn("[function] 企查查日报 {} 截图上传至OSS失败！", fileKey);
                            }
                        }
                    ...
                    String fileKey = (latestNumber+1) + ".png";
                    String outputPathAndFilename = QICHACHA_NEWS_PATH_PREFIX + fileKey;
    ...
    private void initQichachaNewsFileAndRedisMap(){
        ...
        try {
            if(DateTimeUtils.checkTimeHourPeriod(calendarNow, hours) == 2){
                ...
                AliOSSUtils.putFileObject(QICHACHA_NEWS_PATH_PREFIX+(latestNumber+1)+".png", OSS_FOLDER_KEY, (latestNumber+1)+".png");
            }
            if(DateTimeUtils.checkTimeHourPeriod(calendarNow, hours) >= 1){
                ...
                AliOSSUtils.putFileObject(QICHACHA_NEWS_PATH_PREFIX+(latestNumber)+".png", OSS_FOLDER_KEY, (latestNumber)+".png");
            }
            PhantomjsUtils.screenCapture(QICHACHA_EVENING_NEWS_URL+(latestNumber-1), QICHACHA_NEWS_PATH_PREFIX+(latestNumber-1)+".png", 650, 6000, 1.3f, 5000);
            AliOSSUtils.putFileObject(QICHACHA_NEWS_PATH_PREFIX+(latestNumber-1)+".png", OSS_FOLDER_KEY, (latestNumber-1)+".png");
            ...
            AliOSSUtils.putFileObject(QICHACHA_NEWS_PATH_PREFIX+(latestNumber-2)+".png", OSS_FOLDER_KEY, (latestNumber-2)+".png");
        } catch (IOException | InterruptedException ex){
    ...
    public static void main(String[] args) {
        ...
        try {
            if(PhantomjsUtils.screenCapture(url, outputPathAndFilename, 650, 6000, 1.3f, 5000)){
                if(AliOSSUtils.putFileObject(outputPathAndFilename, OSS_FOLDER_KEY, latestNumber+".png")){
                    System.out.println("AliOSS文件上传完成。");
                }
            }
        } catch (IOException | InterruptedException ex){
            ex.printStackTrace();
